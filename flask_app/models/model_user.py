...
from flask_app import bcrypt, DATABASE, session
...
'''
Simple user class for the purpose of practice with validation and password
hashing
'''
class User:
    ...
    '''
    Gets user by their ID number. If logged in, the user_id stored in session
    is passed to it.
    '''
    @classmethod
    def get_user_by_id(cls, user_id) -> 'User':
        data = { 'id': user_id }
    ...
    '''
    Search for users with this email, for the purposes of registration
    validation (i.e. avoid duplicate accounts) and login.
    '''
    @classmethod
    ...
    that use the form's data, the password is hashed and combined with the
